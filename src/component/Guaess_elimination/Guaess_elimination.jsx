...
for (let i = n - 1; i >= 0; i--) {
  ...
  return (
    ...
    <center><h1>Gaussian Elimination</h1></center> {/* Updated title here */}

      <div className='inputmx'>
        <TextField  label="Enter Matrix:" type="number" value={desiredRowCount}  onChange={(e) => setDesiredRowCount(Number(e.target.value))}min={2}/>
      </div>
      ...
      {solutions.length > 0 && (
        <div className='inputmx'>
          ...
          {solutions.map((solution, idx) => (
            <h3><t key={idx}>x{idx + 1} = {solution.toFixed(4)}&nbsp;</t></h3>
          ))}
        </div>
      )}
    </div>
